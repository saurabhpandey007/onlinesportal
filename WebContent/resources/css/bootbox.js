				...
				function init(i, c) {
					...
					function b(q) {
						...
						k(s, function(v, u, t) {
							...
							if (i.type(u) !== "object") {
								throw new Error("button with key " + v
										+ " must be an object")
							}
							if (!u.label) {
								u.label = v
					...
					h.confirm = function() {
						var q;
						q = e("confirm", [ "cancel", "confirm" ], [ "message",
								"callback" ], arguments);
					...
					h.prompt = function() {
						...
						y = n(l(t, arguments, [ "title", "callback" ]), [
								"cancel", "confirm" ]);
						...
						case "select":
							...
							k(u, function(z, A) {
								...
								if (A.group) {
									if (!r[A.group]) {
										r[A.group] = i("<optgroup/>").attr(
												"label", A.group)
									}
									B = r[A.group]
								}
								B.append("<option value='" + A.value + "'>"
										+ A.text + "</option>")
							});
							k(r, function(z, A) {
								w.append(A)
					...
					h.addLocale = function(r, q) {
						i.each([ "OK", "CANCEL", "CONFIRM" ], function(t, s) {
							if (!q[s]) {
								throw new Error(
										"Please supply a translation for '" + s
												+ "'")
							}
						});
						a[r] = {
